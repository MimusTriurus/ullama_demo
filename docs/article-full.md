...
> *Почему DataAsset*
>
> Всё, что отличает одного NPC от другого, должно храниться в одном месте (NPC registry), быть доступным в редакторе и иметь относительно удобный способ редактирования\добавления игровых персонажей.
>
> **DataAsset** - это ассет-контейнер данных в Unreal. Для добавления нового персонажа GD просто меняет значения полей в шаблоне NPC.
>
> * Описание характера персонажа - многострочное текстовое поле.
>
> * Голос - пять слотов под сэмплы (`Neutral`, `Happy`, `Sad`, `Angry`, `Surprise`), в которые перетаскиваются звуковые ассеты.
>
> * Новое действие - элемент в массиве: GD задаёт его сигнатуру (`SellItem(item)`), перечисляет возможные значения каждого параметра (`item: pistol, shotgun, medkit`) и выбирает две реализации - **DataGetter**, который на этот запрос поставит контекст из игры ([раздел 2](#%D0%BE%D1%82%D0%BA%D1%83%D0%B4%D0%B0-%D0%B1%D0%B5%D1%80%D1%91%D1%82%D1%81%D1%8F-%D0%BA%D0%BE%D0%BD%D1%82%D0%B5%D0%BA%D1%81%D1%82)), и **ActionHandler**, который исполнит действие в игровой логике ([раздел 4](#%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D0%BD%D0%B5%D0%BD%D0%B8%D0%B5-actionhandler---unreal-gamelogic)).
>
> Ошибиться типом редактор не даст - в слот голоса не положить меш, в слот обработчика не выбрать класс, который обработчиком не является: в выпадающем списке просто не будет ничего лишнего.
