...
- **Expressive Agent** — NPC анализирует тональность запроса игрока (не опечален ли) и состояние игрока (не ранен ли) и вместе с текстовым ответом возвращает тэг эмоции (соответствующий тональности ответа). Этот тэг определяет, какую анимацию лица (MetaHuman) применить и какой сэмпл голоса использовать для клонирования при преобразовании текста в речь (TTS).

Таким образом, теперь NPC не просто говорит, но и **знает, чувствует и действует**.
...
Отсюда ответ на главный вопрос роста: добавление нового NPC сводится к заполнению полей в DataAsset. Не надо переписывать движок. **Персонаж становится контентом, а не кодом.**
