...
Действие NPC — это **результат оценки запроса игрока и состояния игрового мира (state_of_user, state_of_npc)**.

Всё, что нужно LLM для выбора, лежит в **system prompt**. Он делает две вещи:

* **перечисляет доступные действия** и описывает, в каком случае какое вызывать (например, `DoNothing` — когда вопрос вообще не по адресу торговца), вместе со строгими требованиями к параметрам каждого;

* **задаёт форму ответа** — вернуть *один json объект определенной структуры* `emotion` + `answer` + `action`, а не plain-текст.

Дальше LLM анализирует запрос игрока вместе с контекстом (см. раздел 2), выбирает подходящее действие, подбирает параметры и возвращает формализованный json:
...
## Проблема: промпт — это просьба, а не гарантия

Так оно работает в идеале. На практике всё, что написано в system prompt, для LLM не более чем пожелание. И именно здесь начинаются проблемы, за которые все не любят "LLM в проде":
...
Значит, к промпту нужен второй "ограничитель", который уже не просит, а **запрещает** (вместе они образуют этакий **harness** вокруг LLM). Эту роль играет **валидация output токенов** — грамматики **GBNF** из `llama.cpp`.

Работают они радикально: мы не *просим* LLM выдать валидный JSON, мы **запрещаем ей на уровне сэмплинга генерировать что-либо, кроме валидного JSON нужной формы**.
