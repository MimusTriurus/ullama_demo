...
Игрок хамит — по одному тону выходит `Angry`. Но он ранен, а в кошельке ноль: LLM взвешивает всё вместе и отвечает `Sad` — торговцу его скорее жалко, чем обидно. Одна и та же фраза в разном состоянии даёт разную реакцию.
