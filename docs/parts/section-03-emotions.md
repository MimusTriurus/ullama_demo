...
В DataAsset персонажа, в поле `NpcVoices`, лежат **сэмплы его голоса с разными эмоциями** — по одному на каждый тэг: `Neutral`, `Happy`, `Sad`, `Angry`, `Surprise`. И это **референсы для клонирования**, а не готовые реплики. Пришёл тэг `Happy` — берём радостный сэмпл как образец тембра и настроения и **в реальном времени синтезируем этим голосом сам текст ответа** из поля `answer`.

Так NPC произносит любую сгенерированную фразу своим узнаваемым голосом и с нужной эмоцией, без всякой привязки к набору записанных заранее строк. Параллельно тэг включает мимику, а lipsync подгоняет губы под получившееся аудио.
